...
public class KeyboardView extends View {
    ...
    public void setKeyboard(@Nonnull final Keyboard keyboard) {
        ...
        computeRowShifts(keyboard);
        
        invalidateAllKeys();
        requestLayout();
    }

    private void computeRowShifts(@Nonnull final Keyboard keyboard) {
        int maxRow = 0;
        for (Key key : keyboard.getSortedKeys()) {
            if (key.getRow() > maxRow) {
                maxRow = key.getRow();
            }
        }
        int rowCount = maxRow + 1;
        mRowShifts = new int[rowCount];
        if (rowCount <= 2) {
            return;
        }

        Key[] repKeys = new Key[rowCount];
        for (Key key : keyboard.getSortedKeys()) {
            int r = key.getRow();
            boolean isNormal = !key.isActionKey() && !key.isModifier() && key.getCode() > Constants.CODE_SPACE;
            if (repKeys[r] == null || (isNormal && (repKeys[r].isActionKey() || repKeys[r].isModifier() || repKeys[r].getCode() <= Constants.CODE_SPACE))) {
                repKeys[r] = key;
            }
        }

        org.futo.inputmethod.latin.settings.SettingsValues settings = org.futo.inputmethod.latin.settings.Settings.getInstance().getCurrent();
        float heightScale = settings != null ? settings.mKeyboardHeightScale : 1.0f;
        if (mDrawableProvider != null) {
            heightScale *= mDrawableProvider.getKeyHeightScale();
        }
        float[] ratios = org.futo.inputmethod.v2keyboard.KeyboardSizingCalculatorKt.getKeyboardResizeRatios(getContext());
        float heightRatio = ratios[1];

        float[] drawY = new float[rowCount];
        float[] drawBottom = new float[rowCount];
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            final KeyDrawingConfiguration kdc = mDrawableProvider.selectKeyDrawingConfiguration(keyboard, params, key);
            final Drawable bg = kdc.getBackground();
            ...
            int bgHeight = key.getHeight() + padding.top + padding.bottom;
            
            boolean isBottomRow = (key.getY() + key.getHeight()) >= (keyboard.mOccupiedHeight - key.getHeight() / 2);
            boolean isNumberRow = key.getRow() == 0 && key.getCode() >= '0' && key.getCode() <= '9';
            boolean isFunctional = key.isActionKey() || key.isModifier() || key.getCode() <= Constants.CODE_SPACE || isBottomRow;

            float actualHeightScale = heightScale;
            if (!isFunctional && isNumberRow) {
                actualHeightScale *= 0.85f;
            }

            float scale = 1.0f;
            if (isFunctional && isBottomRow) {
                scale = heightScale * heightRatio * 0.85f;
            } else if (!isFunctional) {
                scale = heightScale * heightRatio * actualHeightScale;
            } else {
                scale = heightScale * heightRatio * 1.0f;
            }

            float offset = (bgHeight - (bgHeight * scale)) / 2.0f;
            drawY[i] = key.getY() - padding.top + offset;
            drawBottom[i] = drawY[i] + bgHeight * scale;
        }

        float[] gaps = new float[rowCount - 1];
        float sumGaps = 0;
        for (int i = 0; i < rowCount - 1; i++) {
            gaps[i] = drawY[i + 1] - drawBottom[i];
            sumGaps += gaps[i];
        }

        float G = sumGaps / (rowCount - 1);

        mRowShifts[0] = 0;
        float currentShift = 0;
        for (int i = 1; i < rowCount; i++) {
            currentShift += G - gaps[i - 1];
            mRowShifts[i] = Math.round(currentShift);
        }
    }
        ...
        if (drawAllKeys || isHardwareAccelerated) {
            ...
            for (final Key key : mInvalidatedKeys) {
                ...
                if (background != null) {
                    ...
                    int rowShift = 0;
                    if (mRowShifts != null && key.getRow() >= 0 && key.getRow() < mRowShifts.length) {
                        rowShift = mRowShifts[key.getRow()];
                    }
                    final int y = key.getY() + getPaddingTop() + rowShift;
    ...
    private void onDrawKey(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        final int keyDrawY = key.getY() + getPaddingTop() + rowShift;
    ...
    protected void onDrawKeyBackground(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        boolean isBottomRow = (key.getY() + key.getHeight()) >= (mKeyboard.mOccupiedHeight - key.getHeight() / 2);
        boolean isNumberRow = key.getRow() == 0 && key.getCode() >= '0' && key.getCode() <= '9';
        
        boolean isFunctional = key.isActionKey() || key.isModifier() || key.getCode() <= Constants.CODE_SPACE || isBottomRow;

        org.futo.inputmethod.latin.settings.SettingsValues settings = org.futo.inputmethod.latin.settings.Settings.getInstance().getCurrent();
        float widthScale = settings != null ? settings.mKeyboardWidthScale : 1.0f;
        float heightScale = settings != null ? settings.mKeyboardHeightScale : 1.0f;
        if (mDrawableProvider != null) {
            heightScale *= mDrawableProvider.getKeyHeightScale();
        }
        ...
        if (!isFunctional) {
            ...
            if (widthScale != 1.0f || widthRatio != 1.0f) {
                int scaledBgWidth = (int) (bgWidth * widthRatio * widthScale);
                bgX += (bgWidth - scaledBgWidth) / 2;
                bgWidth = scaledBgWidth;
            }

            if (actualHeightScale != 1.0f || heightRatio != 1.0f) {
                int scaledBgHeight = (int) (bgHeight * heightRatio * actualHeightScale);
                bgY += (bgHeight - scaledBgHeight) / 2;
                bgHeight = scaledBgHeight;
            }
            bgY += shiftPixels;
        } else {
            if (isBottomRow) {
                int scaledBgHeight = (int) (bgHeight * heightRatio * 0.85f);
                bgY += (bgHeight - scaledBgHeight) / 2;
                bgHeight = scaledBgHeight;
            } else {
                bgY += shiftPixels;
