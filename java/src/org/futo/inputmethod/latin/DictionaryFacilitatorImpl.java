    ...
    @Nonnull public SuggestionResults getSuggestionResults(ComposedData composedData,
        ...
        for(DictionaryGroup dictionaryGroup : mDictionaryGroups) {
            for (final String dictType : ALL_DICTIONARY_TYPES) {
                ...
                if (dictionaryGroup.mLocale != null &&
                        dictionaryGroup.mLocale.getLanguage().equals("vi") &&
                        dictType.equals(Dictionary.TYPE_MAIN) &&
                        !composedData.mIsBatchMode &&
                        composedData.mTypedWord != null &&
                        !composedData.mTypedWord.isEmpty()) {

                    // Tiếng Việt dùng engine thuần Kotlin trong RAM (prefix Telex + sửa lỗi cân
                    // tần suất + ngữ cảnh bigram), KHÔNG dùng bộ giải mã không gian JNI cho TYPE_MAIN.
                    CharSequence prevWordCs = ngramContext.getNthPrevWord(1);
                    String prevWord = prevWordCs == null ? null : prevWordCs.toString();
                    ArrayList<SuggestedWordInfo> viSuggestions =
                            org.futo.inputmethod.latin.suggestions.VietnameseSuggestionEngine.INSTANCE
                                    .getSuggestions(composedData.mTypedWord, prevWord, dictionary);
                    suggestionResults.addAll(viSuggestions);
                    if (null != suggestionResults.mRawSuggestions) {
                        suggestionResults.mRawSuggestions.addAll(viSuggestions);
                    }
                    continue;
                }
