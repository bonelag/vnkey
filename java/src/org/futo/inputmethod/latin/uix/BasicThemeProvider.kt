...
val KeyboardCornerRadiusSetting = SettingsKey(floatPreferencesKey("pref_keyboard_corner_radius"), 36f)
val KeyboardCornerRadiusSettingLandscape = SettingsKey(floatPreferencesKey("pref_keyboard_corner_radius_landscape"), 36f)
...
class BasicThemeProvider(val context: Context, val colorScheme: KeyboardColorScheme) :
    ...
    override val keyHeightScale: Float
    ...
    private fun createKeyDrawable(
        ...
        val mainDrawable = GradientDrawable().apply {
            ...
            if (borderSizePx > 0f && borderColor != null) {
                setStroke(borderSizePx.toInt(), borderColor)
            }
        }
        if (shadowSizePx > 0f) {
            ...
            val layers = arrayOf(shadowDrawable, mainDrawable)
            return LayerDrawable(layers).apply {
                setLayerInset(0, 0, shadowSizePx.toInt(), 0, 0)
                setLayerInset(1, 0, 0, 0, shadowSizePx.toInt())
            }
        }
        return mainDrawable
    }

    private fun makeVisualStyle(
        background: Int,
        foreground: Int,
        highlight: Int,
        foregroundPressed: Int,
        roundedness: Dp,
        borderSize: Float = colorScheme.extended.advancedThemeOptions.keyBorderSize,
        borderColor: Int? = colorScheme.extended.advancedThemeOptions.keyBorderColor?.toArgb(),
        shadowSize: Float = colorScheme.extended.advancedThemeOptions.keyShadowSize
    ): VisualStyleDescriptor {
        val radiusPx = dp(roundedness)
        val borderPx = dp(borderSize.dp)
        val shadowPx = dp(shadowSize.dp)

        val bg = createKeyDrawable(background, radiusPx, borderPx, borderColor, shadowPx)
        val bgPressed = createKeyDrawable(
            Color(highlight).compositeOver(Color(background)).toArgb(),
            radiusPx, borderPx, borderColor, shadowPx
        )
        ...
        val flickBgDrawable = createKeyDrawable(flickBg, radiusPx, borderPx, borderColor, shadowPx)
        ...
        return VisualStyleDescriptor(
            ...
            backgroundDrawableFlicking = buildMap {
                Direction.entries.forEach {
                    put(it, FlickClipDrawable(bgPressed, flickBgDrawable, it))
                }
                put(null, bgPressed)
            }
        )
    ...
    init {
        ...
        keyHeightScale = advanced.keyboardHeightScale
        ...
        actionBarColor = advanced.toolbarColor ?: if(keyBorders) {
        ...
        val isLandscape = sizeStateProvider?.currentSizeState == KeyboardSizeSettingKind.Landscape
        val customRadiusPercent = if (isLandscape) {
            context.getSettingBlocking(KeyboardCornerRadiusSettingLandscape)
        } else {
            context.getSettingBlocking(KeyboardCornerRadiusSetting)
        }
        val baseRadius = 25.dp * (customRadiusPercent / 100f) * advanced.keyRoundness

        val keyCornerRadius = baseRadius
        val spaceCornerRadius = baseRadius
        val actionKeyRadius = baseRadius

        keyStyles = mapOf(
            KeyVisualStyle.Action to run {
                val customBg = advanced.functionalKeyColor?.toArgb()
                val customFg = advanced.onFunctionalKeyColor?.toArgb()
                val customBorderColor = advanced.functionalKeyBorderColor?.toArgb()
                if (customBg != null) {
                    makeVisualStyle(
                        background = customBg,
                        foreground = customFg ?: onPrimary,
                        highlight = highlight,
                        foregroundPressed = highlightForeground,
                        roundedness = actionKeyRadius,
            ...
            KeyVisualStyle.Functional to run {
                val customBg = advanced.functionalKeyColor?.toArgb()
                val customFg = advanced.onFunctionalKeyColor?.toArgb()
                val customBorderColor = advanced.functionalKeyBorderColor?.toArgb()
                if (customBg != null) {
                    makeVisualStyle(
                        background = customBg,
                        foreground = customFg ?: onKeyColor,
                        highlight = highlight,
                        foregroundPressed = highlightForeground,
                        roundedness = keyCornerRadius,
                        borderColor = customBorderColor
                    )
                } else if(keyBorders) {
                    makeVisualStyle(
            ...
            KeyVisualStyle.StickyOff to run {
                val customBg = advanced.functionalKeyColor?.toArgb()
                val customFg = advanced.onFunctionalKeyColor?.toArgb()
                val customBorderColor = advanced.functionalKeyBorderColor?.toArgb()
                if (customBg != null) {
            ...
            KeyVisualStyle.Spacebar to run {
                val customBg = advanced.spacebarColor?.toArgb()
                val customFg = advanced.onSpacebarColor?.toArgb()
                val customBorderColor = advanced.spacebarBorderColor?.toArgb()
                if (customBg != null) {
                    makeVisualStyle(
                        background = customBg,
                        foreground = customFg ?: onKeyColor,
        ...
        val popupBgColor = advanced.popupColor?.toArgb() ?: colorScheme.keyboardPress.toArgb()
        val popupFgColor = advanced.onPopupColor?.toArgb() ?: colorScheme.onKeyboardContainer.toArgb()

        keyFeedback = KeyBackground(
            foregroundColor = popupFgColor,
            padding = Rect(0,0,0,0),
            background = GradientDrawable(
                GradientDrawable.Orientation.TOP_BOTTOM,
                intArrayOf(popupBgColor, popupBgColor),
            ).apply {
                cornerRadius = dp(keyCornerRadius)
            }
        )

        colors[R.styleable.Keyboard_Key_keyPreviewTextColor] = popupFgColor
