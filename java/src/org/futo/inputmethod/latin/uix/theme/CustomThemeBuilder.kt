...
data class CustomThemeBuilderConfiguration(
    ...
    fun build(): SerializableCustomTheme {
        ...
        return initBasicSerializableThemeFromKeyboardScheme(colorScheme).let {
            if(backgroundImagePath != null || !gradientColors.isNullOrEmpty()) {
                val alpha = if (backgroundImagePath != null) (1.0f - backgroundImageOpacity.pow(2)) * 0.32f + 0.5f else 1.0f
                it.copy(
                    backgroundImage = backgroundImagePath,
                    backgroundImageOpacity = backgroundImageOpacity,
                    thumbnailImage = thumbnailImagePath,
                    thumbnailScale = thumbnailImageScale,
                    backgroundImageCropping = if (backgroundImagePath != null) listOf(backgroundImageRect.left, backgroundImageRect.top, backgroundImageRect.right, backgroundImageRect.bottom) else emptyList(),
