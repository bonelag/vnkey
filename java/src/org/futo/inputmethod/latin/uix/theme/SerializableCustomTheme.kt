...
data class SerializableCustomTheme(
    ...
    val keyBorderColor: SerializableColor? = null,
    val spacebarColor: SerializableColor? = null,
    val onSpacebarColor: SerializableColor? = null,
    val spacebarBorderColor: SerializableColor? = null,
    val functionalKeyColor: SerializableColor? = null,
    val onFunctionalKeyColor: SerializableColor? = null,
    val functionalKeyBorderColor: SerializableColor? = null,
    val toolbarColor: SerializableColor? = null,
    val onToolbarColor: SerializableColor? = null,
    val popupColor: SerializableColor? = null,
    val onPopupColor: SerializableColor? = null,
    ...
    fun toKeyboardScheme(ctx: ThemeDecodingContext): KeyboardColorScheme {
        return KeyboardColorScheme(
            ...
            extended = ExtraColors(
                ...
                keyboardBackgroundGradient = if (!gradientColors.isNullOrEmpty()) {
                    try {
                        val colorsList = gradientColors.map { Color(android.graphics.Color.parseColor(it)) }
                        val start = when (gradientDirection) {
                            1 -> androidx.compose.ui.geometry.Offset(0f, Float.POSITIVE_INFINITY)
                            2 -> androidx.compose.ui.geometry.Offset(0f, 0f)
                            3 -> androidx.compose.ui.geometry.Offset(Float.POSITIVE_INFINITY, 0f)
                            else -> androidx.compose.ui.geometry.Offset(0f, 0f)
                        }
                        val end = when (gradientDirection) {
                            1 -> androidx.compose.ui.geometry.Offset(0f, 0f)
                            2 -> androidx.compose.ui.geometry.Offset(Float.POSITIVE_INFINITY, 0f)
                            3 -> androidx.compose.ui.geometry.Offset(0f, 0f)
                            else -> androidx.compose.ui.geometry.Offset(0f, Float.POSITIVE_INFINITY)
                        }
                        androidx.compose.ui.graphics.Brush.linearGradient(colorsList, start, end)
                    } catch (e: Exception) {
                        backgroundImage?.let {
                ...
                advancedThemeOptions = AdvancedThemeOptions(
                    ...
                    keyBorderSize = keyBorderSize,
                    keyShadowSize = keyShadowSize,
                    keyBorderColor = keyBorderColor?.toColor(),
                    spacebarColor = spacebarColor?.toColor(),
                    onSpacebarColor = onSpacebarColor?.toColor(),
                    spacebarBorderColor = spacebarBorderColor?.toColor(),
                    functionalKeyColor = functionalKeyColor?.toColor(),
                    onFunctionalKeyColor = onFunctionalKeyColor?.toColor(),
                    functionalKeyBorderColor = functionalKeyBorderColor?.toColor(),
                    toolbarColor = toolbarColor?.toColor(),
                    onToolbarColor = onToolbarColor?.toColor(),
                    popupColor = popupColor?.toColor(),
                    onPopupColor = onPopupColor?.toColor(),
...
fun initBasicSerializableThemeFromKeyboardScheme(scheme: KeyboardColorScheme): SerializableCustomTheme {
    return SerializableCustomTheme(
        ...
        keyRoundness               = scheme.extended.advancedThemeOptions.keyRoundness,
        keyBorders                 = scheme.extended.advancedThemeOptions.keyBorders ?: true,
        keyBorderSize              = scheme.extended.advancedThemeOptions.keyBorderSize,
        keyShadowSize              = scheme.extended.advancedThemeOptions.keyShadowSize,
        keyBorderColor             = scheme.extended.advancedThemeOptions.keyBorderColor?.toSColor(),
        spacebarColor              = scheme.extended.advancedThemeOptions.spacebarColor?.toSColor(),
        onSpacebarColor            = scheme.extended.advancedThemeOptions.onSpacebarColor?.toSColor(),
        spacebarBorderColor        = scheme.extended.advancedThemeOptions.spacebarBorderColor?.toSColor(),
        functionalKeyColor         = scheme.extended.advancedThemeOptions.functionalKeyColor?.toSColor(),
        onFunctionalKeyColor       = scheme.extended.advancedThemeOptions.onFunctionalKeyColor?.toSColor(),
        functionalKeyBorderColor   = scheme.extended.advancedThemeOptions.functionalKeyBorderColor?.toSColor(),
        toolbarColor               = scheme.extended.advancedThemeOptions.toolbarColor?.toSColor(),
        onToolbarColor             = scheme.extended.advancedThemeOptions.onToolbarColor?.toSColor(),
        popupColor                 = scheme.extended.advancedThemeOptions.popupColor?.toSColor(),
        onPopupColor               = scheme.extended.advancedThemeOptions.onPopupColor?.toSColor(),
        keyboardHeightScale        = scheme.extended.advancedThemeOptions.keyboardHeightScale,
    )
}
