...
class DataStoreHelper {
    ...
    companion object {
        ...
        fun init(context: Context) {
            ...
            // Đọc cache ban đầu từ SharedPreferences
            try {
                val sharedPrefs = context.getSharedPreferences("datastore_cache", Context.MODE_PRIVATE)
                val mutablePrefs = mutablePreferencesOf()
                sharedPrefs.all.forEach { (name, value) ->
                    if (value != null) {
                        try {
                            when (value) {
                                is Boolean -> mutablePrefs[booleanPreferencesKey(name)] = value
                                is Int -> mutablePrefs[intPreferencesKey(name)] = value
                                is Long -> mutablePrefs[longPreferencesKey(name)] = value
                                is Float -> mutablePrefs[floatPreferencesKey(name)] = value
                                is String -> mutablePrefs[stringPreferencesKey(name)] = value
                                is Set<*> -> {
                                    @Suppress("UNCHECKED_CAST")
                                    mutablePrefs[stringSetPreferencesKey(name)] = value as Set<String>
                                }
                            }
                        } catch (e: Exception) {
                            Log.e("DataStoreHelper", "Error parsing cached setting $name", e)
                        }
                    }
                }
                currentPreferences = mutablePrefs
            } catch (e: Exception) {
                Log.e("DataStoreHelper", "Failed to load cached settings", e)
            }

            // Thu thập và lưu cache bất đồng bộ
            GlobalScope.launch(Dispatchers.IO) {
                context.dataStore.data.collect { prefs ->
                    ...
                    try {
                        ...
                        Log.e("DataStoreHelper", "Failed to cache settings", e)
                    }
                }
            }
        }
...
// Mức tự sửa lỗi tiếng Việt: 0 = Thấp, 1 = Cân bằng (mặc định), 2 = Nghiêm ngặt
val VIETNAMESE_AUTOCORRECT_LEVEL = SettingsKey(
    key = intPreferencesKey("vietnameseAutocorrectLevel"),
    default = 1
)