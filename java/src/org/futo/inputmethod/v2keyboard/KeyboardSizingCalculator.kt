    ...
    fun calculate(layoutName: String, settings: SettingsValues): ComputedKeyboardSize? {
        ...
        val numRows = 4.0 +
                ...
                when { // Number row height
                    ...
                    // If it's enabled by user, add only 0.4 to make the keyboard slightly less tall
                    settings.mIsNumberRowEnabled -> 0.24
...
fun getKeyboardResizeRatios(context: Context): FloatArray {
    try {
        val sizeState = (context as KeyboardSizeStateProvider).currentSizeState
        val savedSettingsStr = context.getSettingBlocking(KeyboardSettings[sizeState]!!)
        val savedSettings = SavedKeyboardSizingSettings.fromJsonString(savedSettingsStr) 
            ?: return floatArrayOf(1.0f, 1.0f)
        
        val defaultSettings = getDefaultSettingForKind(sizeState, context)
        val hRatio = defaultSettings.heightMultiplier / savedSettings.heightMultiplier.guardNaN(1.0f)
        
        val density = context.resources.displayMetrics.density
        val width = ResourceUtils.getDefaultKeyboardWidth((context as? LatinIME)?.window?.window, context.resources)
        
        val defaultOcc = width - (defaultSettings.paddingDp.left.value * density) - (defaultSettings.paddingDp.right.value * density)
        val currentOcc = width - (savedSettings.paddingDp.left.value * density) - (savedSettings.paddingDp.right.value * density)
        
        val wRatio = if (currentOcc > 0) (defaultOcc / currentOcc) else 1.0f
        
        return floatArrayOf(wRatio, hRatio)
    } catch (e: Exception) {
        return floatArrayOf(1.0f, 1.0f)
    }
}