...
object LayoutManager {
    ...
    private fun getAllLayoutPaths(context: Context): List<String> {
        return try {
            context.assets.open("layouts_index.txt").use { inputStream ->
                inputStream.bufferedReader().readLines().filter { it.isNotBlank() }
            }
        } catch (e: Exception) {
            Log.w("LayoutManager", "layouts_index.txt not found, falling back to asset listing", e)
            val assetManager = context.assets
            listFilesRecursively(assetManager, "layouts").filter {
    ...
    fun init(context: Context) {
        ...
        val layoutPaths = getAllLayoutPaths(context)
    ...
    fun getAllLayoutNames(context: Context): List<String> {
        ensureInitialized()
        return getAllLayoutPaths(context).map {
